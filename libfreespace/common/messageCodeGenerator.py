...
class MessageCodeGenerator:
    ...
    def writeMessages(self, messages):
        messages.sort(key = cmp_to_key(compareMessages))
...
def cTypeToTypeInfo(ct, sizeInBytes):
    ...
    if ct == 'uint32_t':
        ...
        typeInfo['count'] = int(sizeInBytes / 4)
        typeInfo['width'] = 4
    elif ct == 'uint16_t':
        typeInfo['signed'] = False
        typeInfo['count'] = int(sizeInBytes / 2)
        typeInfo['width'] = 2
    elif ct == 'uint8_t':
        typeInfo['signed'] = False
        typeInfo['count'] = int(sizeInBytes / 1)
        typeInfo['width'] = 1
    elif ct == 'int32_t':
...
def main(argv=None):
    ...
    try:
        ...
        for f in args.messageFiles:
            # execfile(f, g, d)
            exec(compile(open(f,"rb").read(), f, 'exec'), g, d)
